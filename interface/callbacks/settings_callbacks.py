...
async def show_settings(callback: types.CallbackQuery, state: FSMContext):
    ...
    text, reply_markup = get_settings_list(api_errors, transaction_info, interface_info, debug, db_info, min_other_token_value)
    msg = await callback.message.edit_text(text, reply_markup=reply_markup, disable_web_page_preview=True)
    await state.update_data(settings_message_id=msg.message_id, api_errors=api_errors, transaction_info=transaction_info, interface_info=interface_info, debug=debug, db_info=db_info, min_other_token_value=min_other_token_value)
    await callback.answer()
...
async def edit_setting_start(callback: types.CallbackQuery, state: FSMContext):
    ...
    current_value = db.settings.get_setting(setting_name, "0")  # По умолчанию для переключателей
    if setting_name == "MIN_OTHER_TOKEN_VALUE":
...
async def toggle_setting(callback: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        await state.update_data(settings_message_id=msg.message_id, api_errors=api_errors, transaction_info=transaction_info, interface_info=interface_info, debug=debug, db_info=db_info, min_other_token_value=min_other_token_value)
    await state.update_data(api_errors=api_errors, transaction_info=transaction_info, interface_info=interface_info, debug=debug, db_info=db_info, min_other_token_value=min_other_token_value)
    await callback.answer()
...
async def process_setting_value(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        try:
            ...
            await state.update_data(settings_message_id=msg.message_id, api_errors=data.get("api_errors", "❌ВЫКЛ"), transaction_info=data.get("transaction_info", "✅ВКЛ"), interface_info=data.get("interface_info", "❌ВЫКЛ"), debug=data.get("debug", "❌ВЫКЛ"), db_info=data.get("db_info", "❌ВЫКЛ"), min_other_token_value=db.settings.get_setting("MIN_OTHER_TOKEN_VALUE", "50"))
        await state.update_data(min_other_token_value=db.settings.get_setting("MIN_OTHER_TOKEN_VALUE", "50"))
        await state.clear()
