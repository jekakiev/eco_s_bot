...
def get_keyboard_functions():
    ...
    return get_main_menu, get_wallets_list, get_wallet_control_keyboard, get_tokens_keyboard, get_back_button
...
async def show_wallets(callback: types.CallbackQuery, state: FSMContext):
    ...
    get_main_menu, get_wallets_list, _, _, _ = get_keyboard_functions()
...
async def add_wallet_start(callback: types.CallbackQuery, state: FSMContext):
    ...
    _, _, _, _, get_back_button = get_keyboard_functions()
...
async def process_wallet_name(message: types.Message, state: FSMContext):
    ...
    if not tracked_tokens:
        get_main_menu, _, _, _, _ = get_keyboard_functions()
        ...
        _, _, _, get_tokens_keyboard, _ = get_keyboard_functions()
...
async def save_tokens(callback: types.CallbackQuery, state: FSMContext):
    ...
    _, _, get_wallet_control_keyboard, _, _ = get_keyboard_functions()
    await state.clear()
