...
class TransactionsDB:
    ...
    def add_transaction(self, wallet_address, transaction_hash, timestamp, block_number, amount_usd):
        try:
            self.cursor.execute("""
                INSERT INTO transactions (wallet_address, transaction_hash, timestamp, block_number, amount_usd)
                VALUES (%s, %s, %s, %s, %s)
                ON DUPLICATE KEY UPDATE is_processed = FALSE
            """, (wallet_address, transaction_hash, timestamp, block_number, amount_usd))
            self.connection.commit()
            logger.info(f"Транзакция добавлена: {transaction_hash}")
        except Error as e:
            logger.error(f"Ошибка добавления транзакции: {str(e)}")
            self.connection.rollback()

    def update_transaction(self, transaction_hash, data):
        try:
            set_clause = ", ".join(f"{key} = %s" for key in data.keys())
            values = list(data.values()) + [transaction_hash]
            self.cursor.execute(f"UPDATE transactions SET {set_clause} WHERE transaction_hash = %s", values)
            self.connection.commit()
            logger.info(f"Транзакция обновлена: {transaction_hash}")
        except Error as e:
